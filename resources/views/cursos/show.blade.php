...
<head>
    ...
    <style>
        ...
        /* Diseño moderno para la sección de calificación */
        .calificacion-panel {
    ...
    <div class="container mx-auto my-6">
            ...
            <div class="leccion-botones">
                @foreach ($curso->lecciones as $leccion)
                    <div class="flex items-center gap-2">
                        ...
                        <input type="checkbox" data-leccion-id="{{ $leccion->id }}" class="progreso-checkbox"
                            {{ $leccion->progresos->where('alumno_id', auth('alumnoe')->id())->first()?->completado ? 'checked' : '' }}>

                        {{-- Botón para la lección --}} 
                        <form action="{{ route('cursos.showLeccion', [$curso->id, $leccion->id]) }}" method="GET"
                            class="flex-1">
        ...
        @if ($curso->examen)
            ...
            @php
                ...
                $resultadoExistente = \App\Models\ResultadoExamen::where('alumno_id', $alumnoId)
                    ->where('exam_id', $curso->examen->id)
                    ->first();
            @endphp

            <a href="{{ $resultadoExistente ? '#' : route('exams.show', $curso->examen->id) }}"
                class="mt-4 inline-block bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 
              {{ $resultadoExistente ? 'opacity-50 cursor-not-allowed' : '' }}"
                {{ $resultadoExistente ? 'disabled' : '' }}
                @if ($resultadoExistente) aria-disabled="true" 
           title="Ya realizaste este examen" @endif>
                {{ $resultadoExistente ? 'Ya realizaste este examen' : 'Ver Examen' }}
            </a>
        @else
        ...
        <div class="calificacion-panel">
            ...
            <form action="{{ route('calificaciones.store', $curso->id) }}" method="POST" id="form-calificacion">
                @csrf
                <div class="flex">
                    <select name="estrellas" id="estrellas" required {{ $calificacionExistente ? 'disabled' : '' }}>
                ...
                {{ $calificacionExistente ? 'bg-red-600 opacity-50 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700' }} 
                text-white py-2 px-4 rounded-lg"
                    {{ $calificacionExistente ? 'disabled' : '' }}
                    @if ($calificacionExistente) aria-disabled="true" 
                title="Ya calificaste este curso" @endif>
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg mt-6">
            ...
            <form action="{{ route('cursos.comentarios.store', ['curso' => $curso->id]) }}" method="POST"
                class="mb-4">
                @csrf
                <textarea name="comentario" class="w-full p-3 rounded border-gray-300" rows="4"
                    placeholder="Escribe tu comentario" required></textarea>
                <button type="submit" class="mt-2 px-4 py-2 bg-indigo-600 text-white rounded">Publicar
                    Comentario</button>
            </form>
